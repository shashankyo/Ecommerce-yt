...
func HashPassword (password string) string{
	bytes, err := bcryptGenerateFromPassword([]byte(password), 14)
	if err != nil {
		log.Panic(err)
	}
	return string(bytes)
...
func Signup () gin.HandlerFunc {
	...
	return func(c *gin.Context){
		...
		if count >0 {
			c.JSON(http.StatusBadRequest, gin.H{"error":"useralready exist"})
		}
		count, err := userCollection.CountDocuments(ctx, bson.M{"phone":user.})
...
func Login () gin.HandlerFunc{
	...
	token, refreshToken, _ := generate.TokenGenerator(*founduser.Email, *founderuser.First_Name, *founduser.Last_Name, *founduser.User_ID)
	defer c2:45:54 / 6:02:43
	￼
	ancel()

	generate.UpodateAllTokens(token, refreshtoken, founduser.User_ID)
